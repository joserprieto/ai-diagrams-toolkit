...
##
## @example
##   source "${REPO_ROOT}/scripts/tests/lib/setup.sh"
##   tests::setup::validate_context
##   tests::setup::load_env "${ENV_FILE}"
##
...
if [[ -n "${tests__setup__LOADED:-}" ]]; then
  return 0
fi
readonly tests__setup__LOADED=1
...
readonly tests__setup__NAMESPACE="tests::setup"
...
##
tests::setup::validate_repo_root() {
  if [[ -z "${REPO_ROOT:-}" ]]; then
...
##
tests::setup::validate_required_vars() {
  local env_file="$1"
...
## @return 0 if valid, exits with 1 if invalid
##
tests::setup::validate_ai_cli() {
  local env_file="$1"

  # Delegate to strategy if it exists
  if declare -f tests::setup::${SETUP_STRATEGY}::validate_context >/dev/null 2>&1; then
    tests::setup::${SETUP_STRATEGY}::validate_context
  fi

  if declare -f tests::setup::${SETUP_STRATEGY}::validate_executor >/dev/null 2>&1; then
    tests::setup::${SETUP_STRATEGY}::validate_executor "${env_file}"
  fi
}
...
tests::setup::initialize() {
  ...
  tests::setup::validate_repo_root
  tests::setup::validate_ai_cli "${env_file}"
  tests::setup::load_env "${env_file}"
  tests::setup::validate_required_vars "${env_file}"
  tests::setup::create_output_dir "${output_dir}"
}
